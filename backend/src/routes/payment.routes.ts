...
paymentRouter.post('/create', async (req, res) => {
    try {
        ...
        console.log("[PAYMENT] POST /create - Error:", error.message);        return res.status(500).json({ 
...
paymentRouter.all('/callback', async (req, res) => {
        ...
        console.log("[PAYMENT] /callback - Error:", error.message);        await failPayment(tx_ref)
        return res.status(400).json({error: "Payment Failed"})
    } catch (error: any) {

        retuconsole.log("[PAYMENT] POST /webhook - Missing signature or secret");
        ...
        console.log("[PAYMENT] POST /webhook - Error:", error.message);                } catch (walletErr) {
                    // Not found in either table?
