...
walletRouter.get("/", authMiddleware, async (req, res) => {
  ...
  try {
    ...
    console.log("[WALLET] POST /transaction - Retrieved", walletTrx.length, "transactions for wallet:", walletId);
    ...
    console.log("[WALLET] POST /topup - Error:", error.message);", async (req, res) => {
  const { userId } = req.body;
  const userWallet = await getWallet(userId);
...
walletRouter.post("/transaction", async (req, res) => {
  const { walletId } = req.body;
  const walletTr"[WALLET] /callback - Wallet callback received for transaction:", tx_ref);
    ...
    console.log(`Wallet Callback received: ${tx_ref}`, {
      boole.log("[WALLET] POST /pay/reservation - Processing reservation payment for user:", userId, "amount: ETB", amount);
    ...
    console.log("[WALLET] POST /pay/reservation -
    ...
    retuole.log("[WALLET] POST /pay/reservation-fee - Processing reservation fee for user:", userId, "amount: ETB", amount);
    ...
    console.log("[WALLET] POST /pay/reservation-fee -
      reservationId,
      amount,
    );
    if (!payRes)
