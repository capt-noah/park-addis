...
import { registerUserAndCar, createSession } from "@/src/services/auth.service";

export async function POST(req: NextRequest) {
    try {
        const { fullName, email, password, phoneNumber, role, car } = await req.json()
        const { plateNumber, carModel, color } = car

        const user = await registerUserAndCar(fullName, email, password, phoneNumber, role, plateNumber, carModel, color)
