...
import { reserveSpot } from "@/backend/src/services/reservation.service";
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!reservation) {
      return NextResponse.json(
        { error: "Failed to create reservation or spot unavailable" },
        { status: 400 },
      );
    }
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    const { cancelReservation } = await import(
      "@/backend/src/services/reservation.service"
    );
