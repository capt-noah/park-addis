...
export default function ReservationsClient({
  ...
  const handleCancel = async (reservationId: string) => {
    showConfirmation({
      ...
      message:
        "Are you sure you want to cancel this reservation? If cancelled within 15 minutes of booking, you'll receive a full refund of the reservation fee.",
      ...
      onConfirm: async () => {
        try {
          ...
          if (response.ok) {
            ...
            if (cancelData.refund && cancelData.refund.refunded) {
              showNotification(
                `Reservation cancelled successfully. Refund of ETB ${cancelData.refund.balance} issued.`,
                "success",
              );
            } else {
      ...
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-muted-foreground text-xs font-medium tracking-tight">
            Manage and view details of your parking history.
          </p>
        </div>
        <Link
          href="/locations"
          className="bg-primary text-white px-5 py-3 rounded-2xl font-bold text-[11px] flex items-center gap-2 hover:opacity-90 transition-all shadow-lg shadow-primary/20"
        >
        ...
        {filteredReservations.length > 0 ? (
          filteredReservations.map((res) => (
            <ReservationCard
              ...
              image="/bole.png"
              active={
                res.status.toUpperCase() === "RESERVED" ||
