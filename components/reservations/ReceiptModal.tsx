    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="relative w-full max-w-[360px] bg-white rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-300">
        ...
        <div className="pt-8 pb-6 px-8 flex flex-col items-center border-b border-dashed border-slate-200 relative">
          ...
          {/* Receipt Title + Total */}
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-2">Payment Receipt</p>
          <h2 className="text-4xl font-extrabold text-slate-900 tracking-tighter">
            <span className="text-xl text-slate-400 mr-1">ETB</span>{totalFee}
          </h2>
        </div>
        ...
        <div className="px-8 py-5 space-y-3 flex-1 relative overflow-hidden">
          <div className="relative z-10 space-y-3">
            ...
            <div className="relative z-10 grid grid-cols-2 gap-4">
              <div>
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Receipt No</p>
                <p className="text-sm font-extrabold text-slate-900">#{reservation.id?.substring(0, 6).toUpperCase()}</p>
              </div>
            ...
            {/* Location + Time */}
            <div className="relative z-10 grid grid-cols-2 gap-4 pb-3 border-b border-slate-100">
              <div>
              ...
              <div className="text-right">
                ...
                <div className="flex items-center justify-end gap-1 text-[#004D40]">
                  <Clock size={12} />
            ...
            <div className="relative z-10 space-y-2">
              <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-100">
                <div className="p-2 bg-white rounded-xl shadow-sm border border-slate-100">
                  <Car size={18} className="text-[#004D40]" />
                </div>
                <div className="flex-1 overflow-hidden">
                  ...
                  <p className="text-[9px] font-medium text-slate-500 uppercase tracking-wider">{reservation.plateNumber || "N/A"}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-100">
                <div className="p-2 bg-white rounded-xl shadow-sm border border-slate-100">
                  <CreditCard size={18} className="text-[#004D40]" />
                </div>
            ...
            {/* Fee Breakdown */}
            <div className="relative z-10 bg-emerald-50/50 px-5 py-3 rounded-2xl border border-emerald-100/50">
              <div className="space-y-2">
                <div>
                  <div className="flex justify-between items-baseline">
                    <span className="text-[9px] font-bold uppercase tracking-wider text-emerald-700/60">Parking ({durationDisplay})</span>
                    <span className="text-xs font-bold text-slate-600">ETB {parkingFee.toFixed(2)}</span>
                  </div>
                  <p className="text-[9px] text-slate-400 mt-0.5">{durationHrs.toFixed(2)} hrs × ETB {baseFeePerHour.toFixed(2)}/hr</p>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-[9px] font-bold uppercase tracking-wider text-emerald-700/60">Reservation Fee</span>
                  <span className="text-xs font-bold text-slate-600">ETB {reservationFee.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center pb-2 border-b border-emerald-100/50">
                  ...
                  <span className="text-xs font-bold text-slate-600">ETB {serviceFee.toFixed(2)}</span>
                </div>
                <div className="flex justify-between items-center pt-1">
                  <span className="text-[10px] font-extrabold uppercase tracking-widest text-[#004D40]">Total Paid</span>
                  <span className="text-xl font-extrabold text-slate-900">ETB {totalFee}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Footer Actions */}
        <div className="px-8 pb-10 pt-2 bg-white relative z-10">
