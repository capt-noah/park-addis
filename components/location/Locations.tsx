  ...
  return (
    <div className="w-full">
      ...
      <>
        <div 
          ref={scrollRef}
          className="flex gap-2 overflow-x-auto pb-2 scroll-smooth [&::-webkit-scrollbar]:hidden snap-x"
        >
          {locationsData.map((loc) => (
            <div 
              id={`location-card-${loc.properties.id}`}
              key={loc.properties.id}
              onClick={() => setSelectedLocationId(loc.properties.id)}
              className={`flex-shrink-0 w-[91vw] sm:w-[335px] md:w-[360px] snap-center cursor-pointer transition-transform duration-300 ${
                selectedLocationId === loc.properties.id ? 'scale-100' : 'scale-95 hover:scale-[0.98]'
              }`}
            >
              <div className={`bg-card/90 dark:bg-card/70 backdrop-blur-xl border rounded-3xl p-3 flex gap-3 h-[150px] ${
                selectedLocationId === loc.properties.id ? 'border-primary' : 'border-border/50'
              }`}>
                {/* Image Section */}
                <div className="w-24 h-24 md:w-28 md:h-28 rounded-2xl overflow-hidden bg-white p-1 border border-border flex-shrink-0 self-center">
                  <div className="relative w-full h-full rounded-xl overflow-hidden">
                    <Image src="/bole.png" alt={loc.properties.name} fill className="object-cover" />
                  </div>
                </div>
                ...
                <div className="flex-1 min-w-0 pt-1 flex flex-col h-full overflow-hidden">
                  <div className="flex justify-between items-start mb-1 gap-2">
                    <h3 className="text-base font-black text-foreground truncate">{loc.properties.name}</h3>
                    <div className="flex items-center gap-1 shrink-0 pt-0.5">
                      <Star className="w-3.5 h-3.5 fill-amber-500 text-amber-500" />
                      <span className="text-[11px] font-bold text-foreground">{loc.properties.ratings / 10}</span>
                    </div>
                  </div>
                  
                  <p className="text-[11px] font-medium text-muted-foreground truncate mb-1.5 leading-tight">{loc.properties.address}</p>
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    <span className="px-2 py-0.5 rounded-md bg-primary/10 text-primary text-[10px] font-black uppercase tracking-wider border border-primary/20">
                      {'Available' === 'Available' ? 'OPEN' : 'FULL'}
                    </span>
                    <div className="flex items-center gap-1.5 text-[11px] font-bold text-muted-foreground shrink-0">
                      <div className="flex items-center gap-1">
                        <Clock className="w-3.5 h-3.5" />
                        <span>~{Math.round(loc.properties.eta)} min</span>
                      </div>
                      <span className="text-muted-foreground/30">•</span>
                      <div className="flex items-center gap-1">
                        <MapPin className="w-3.5 h-3.5" />
                        <span>{Number((parseFloat(String(loc.properties.distance || 0)) / 1000).toFixed(1)).toFixed(1)}km</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between mt-auto pt-1 px-1">
                    <div className="flex flex-col">
                        <span className="text-lg font-black text-foreground leading-none">
                          {Number(loc.properties.price || 0).toFixed(2)}<span className="text-[10px] text-muted-foreground font-bold ml-1">ETB/hr</span>
                        </span>
                    </div>
                    <Link 
                      href={`/locations/${loc.properties.id}`} 
          ...
          {locationsData.map((loc) => (
            <div 
              ...
              className={`h-1.5 rounded-full transition-all duration-300 ${
                selectedLocationId === loc.properties.id ? 'w-6 bg-primary' : 'w-2 bg-primary/50'
              }`}
            />
          ))}
        </div>
