...
export default function LocationsContainer({ locationsData }: {  locationsData: ParkingLocation[] }) {
  ...
  const [distanceFilter, setDistanceFilter] = useState<'All' | 200 | 400 | 600>('All');
  const [displayedLocations, setDisplayedLocations] = useState<ParkingLocation[]>(locationsData);
  ...
  const lng = 38.78243920830075;
    ...
    try {
      const res = await fetch(`/api/locations?distance=${filter}&lat=${lat}&lng=${lng}`);
      if (res.ok) {
        const data = await res.json();
        console.log(data)
        setDisplayedLocations(data);
