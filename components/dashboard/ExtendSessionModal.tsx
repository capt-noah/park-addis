...
export function ExtendSessionModal({ reservation, onClose }: { reservation: any; onClose: () => void }) {
  ...
  const parsedMins = typeof extraMinutes === 'number' ? extraMinutes : (parseInt(extraMinutes as string) || 0);

  const currentEndTimeStr = new Date(reservation.endTime).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' });
  
  // Calculate new end time
  const newEndTime = new Date(reservation.endTime);
  newEndTime.setMinutes(newEndTime.getMinutes() + parsedMins);
  const newEndTimeStr = newEndTime.toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' });
      ...
      <div className="relative w-full max-w-[360px] bg-white rounded-[2.5rem] shadow-2xl p-6 flex flex-col animate-in slide-in-from-bottom-8 duration-300">
        ...
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-extrabold text-slate-900 text-lg tracking-tight">Extend Session</h2>
          </div>
          <button 
            onClick={onClose} 
        ...
        <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 mb-6">
          <div className="flex justify-between items-center mb-3">
            <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Current End Time</span>
            <span className="text-sm font-bold text-slate-700">{currentEndTimeStr}</span>
          </div>
          <div className="flex justify-between items-center pt-3 border-t border-slate-200/60">
            <span className="text-[10px] uppercase font-bold text-[#004D40] tracking-wider">New End Time</span>
            <div className="flex items-center gap-1.5 text-[#004D40]">
              <span className="text-sm font-extrabold">{newEndTimeStr}</span>
              <span className="text-[10px] bg-[#004D40]/10 px-1.5 py-0.5 rounded font-bold">+{parsedMins}m</span>
            </div>
          </div>
        </div>

        {/* Duration Picker */}
        <p className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-3 px-1">Add Time</p>
        <div className="grid grid-cols-3 gap-2 mb-2">
          {[30, 60].map((mins) => (
            <button
              key={mins}
              onClick={() => { setExtraMinutes(mins); setIsCustom(false); }}
          ...
          <div className="flex items-center justify-between bg-slate-50 border border-slate-100 rounded-2xl p-2 mb-4 animate-in fade-in slide-in-from-top-2">
            ...
            <div className="flex-1 flex justify-center items-center">
              <input 
                type="number" 
                value={extraMinutes}
                onChange={(e) => setExtraMinutes(e.target.value === "" ? "" : parseInt(e.target.value))}
                onBlur={() => {
                  if (extraMinutes === "" || parsedMins < 1) setExtraMinutes(15);
                }}
              ...
              />
              <span className="text-xs font-bold text-slate-500 ml-1 mt-0.5">Mins</span>
            </div>
            <button 
        ...
        <div className="flex justify-between items-center px-2 mb-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-slate-600">Additional Cost</span>
          </div>
          <span className="text-lg font-black text-slate-900">ETB {additionalCost.toFixed(2)}</span>
        </div>
